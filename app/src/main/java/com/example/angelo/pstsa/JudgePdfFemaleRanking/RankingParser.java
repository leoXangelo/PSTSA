package com.example.angelo.pstsa.JudgePdfFemaleRanking;
    ...
    protected void onPostExecute(Integer integer) {
        ...
        if(integer==1)
        {
            ...
            try{
                ...
                int x = 0, number;
                int trank = 0;
                int res = db.deleteFemaleRank("Female");
                if (res > 0){
                ...
                for (String name : participant_list){
                    ...
                    Boolean result = db.SaveFemaleRank(number, name, "Female", trank, 0);
                    if (result == true){
            ...
            Cursor res = db.GetFemaleRank();
            if (res != null && res.getCount() > 0){
                while (res.moveToNext()){
                    if (res.getInt(4) == tmp){
                        db.UpdateFemaleRank(String.valueOf(res.getInt(1)), rank);
                        loop++;
                    }else{
                        if (initial == false){
                            db.UpdateFemaleRank(String.valueOf(res.getInt(1)), rank);
            ...
            try {
                ...
                filename = eventName + " Judges Score Ranking [Female] " + timeStamp;
