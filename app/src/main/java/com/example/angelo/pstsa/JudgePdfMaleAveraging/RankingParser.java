package com.example.angelo.pstsa.JudgePdfMaleAveraging;
    ...
    protected void onPostExecute(Integer integer) {
        ...
        if(integer==1)
        {
            ...
            try{
                ...
                int x = 0, number;
                int trank = 0;
                int res = db.deleteMaleAverage("Male");
                if (res > 0){
                ...
                for (String name : participant_list){
                    ...
                    Boolean result = db.SaveMaleAverage(number, name, "Male", trank, 0);
                    if (result == true){
            ...
            Cursor res = db.GetMaleAverage();
            if (res != null && res.getCount() > 0){
                while (res.moveToNext()){
                    if (res.getInt(4) == tmp){
                        db.UpdateMaleAverage(String.valueOf(res.getInt(1)), rank);
                        loop++;
                    }else{
                        if (initial == false){
                            db.UpdateMaleAverage(String.valueOf(res.getInt(1)), rank);
            ...
            try {
                ...
                filename = eventName + " Judges Score Averaging [Male] " + timeStamp;
