package com.example.angelo.pstsa.JudgePdfMaleRanking;
    ...
    protected void onPostExecute(Integer integer) {
        ...
        if(integer == 1) {
            try{
                ...
                int x = 0, number;
                int trank = 0;
                int res = db.deleteMaleRank("Male");
                if (res > 0){
                ...
                for (String name : participant_list){
                    ...
                    for (int i = 0; i < judgeReports.size(); i++){
                        ...
                        if (judgeReport.getPname().equals(name)){
                            trank += judgeReport.getRank();
                        }
                    }
                    Boolean result = db.SaveMaleRank(number, name, "Male", trank, 0);
                    if (result == true){
            ...
            Cursor res = db.GetMaleRank();
            if (res != null && res.getCount() > 0){
                while (res.moveToNext()){
                    if (res.getInt(4) == tmp){
                        db.UpdateMaleRank(String.valueOf(res.getInt(1)), rank);
                        loop++;
                    }else{
                        if (initial == false){
                            db.UpdateMaleRank(String.valueOf(res.getInt(1)), rank);
            ...
            try {
                ...
                filename = eventName + " Judges Score Ranking [Male] " + timeStamp;
                ...
                JudgeReport jd;
                //Sub-Header
                insertCell(table, "MALE", Element.ALIGN_CENTER, judge_list.size() + 5, bf12);
                for (String participant : participant_list){
                    ...
                    for (int i =0; i < judgeReports.size(); i++){
                        jd = judgeReports.get(i);
                        if (participant.equalsIgnoreCase(jd.getPname())){
                            table.addCell(new Phrase(""+jd.getRank(), font3));
                            trank += (double)jd.getRank();
                            check_judge++;
                        }
                    }
                ...
                Paragraph paragraph1 = new Paragraph("JUDGING RESULT [Ranking]" + System.lineSeparator() + "", font);
